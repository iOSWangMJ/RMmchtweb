      ...
      <div class="dates-bg">
        <div class="week-row" v-for="week in currentDates">
          <div class="day-cell" v-for="day in week" track-by="$index"
            ...
            <p class="day-number">{{day.monthDay}} <span v-show='day.isToday'>(今日)</span></p>
            <!--start 日期项目中应用的金额 与下载按钮  start-->
            <p class="day-total"> {{day.total?day.total:"0.00"}}</p>
            <p class="download-event" v-show="day.downloadImg"><a  @click='downloadClick(day.date,$event)'><img :src="downloadImg" style="width:19px;height:19px;" alt=""></a></p>
            <!--end 日期项目中应用的金额 与下载按钮  end-->
          </div>
        </div>
      </div>
      ...
      <!-- <div class="dates-events">
        <div class="events-week" v-for="week in currentDates">
          <div class="events-day" v-for="day in week" track-by="$index"
            ...
            <!--start 日期项目中应用的金额 与下载按钮  start-->
            <!-- <p class="day-total"> {{day.total?day.total:"0.00"}}</p>
            <p class="download-event"><a  @click='downloadClick(day.date,$event)'><img :src="downloadImg" style="width:19px;height:19px;" alt=""></a></p> -->
            <!--end 日期项目中应用的金额 与下载按钮  end-->
  ...
  export default {
    ...
    methods : {
      ...
      getCalendar () {
        ...
        let endDate = dateFunc.getEndDate(current)  //last day of this month
        ...
        for(let perWeek = 0 ; perWeek < 6 ; perWeek++) {
          ...
          for(let perDay = 0 ; perDay < 7 ; perDay++) {
            ...
            //   金额总计
              total:this.settlementInfo(startDate).total,
              downloadImg:this.settlementInfo(startDate).downloadImg
            })

            startDate.setDate(startDate.getDate() + 1)
            // if (perWeek==5 && startDate > endDate) {
            // //   isFinal = true
            //   break
            // }
          }
          if (perWeek==5 && week[0].date>endDate) {
              break
          }
          calendar.push(week)
          // if (isFinal) break
      ...
      settlementInfo(date){
        ...
        for (var i = 0; i < this.events.length; i++) {
            //ie 浏览器下的日期兼容性问题  不支持 yyyy-MM-dd
            let dt = new Date(this.events[i].date.replace(/-/g,'/')).getTime()
             if (date == dt) {
                 total= this.events[i].total
                 if (total!='0.00') {
                     downloadImg =true
                 }
                 break
    ...
    // add  downloadClick function  by wangwenqi
    downloadClick(day,jsEvent){
        jsEvent.stopPropagation()
        this.$emit('downloadClick',day,jsEvent)
    }
